...
public class KalmanFilter {
    ...
    public void reconfigurefmatrix(double deltatime){
        this.F.set(0, 0, 1); this.F.set(0, 2, deltatime); this.F.set(0, 4, 0.5 * deltatime * deltatime); // Position (X)
        this.F.set(1, 1, 1); this.F.set(1, 3, deltatime); this.F.set(1, 5, 0.5 * deltatime * deltatime); // Position (Y)
        this.F.set(2, 2, 1); this.F.set(2, 4, deltatime); // Velocity (X)
        this.F.set(3, 3, 1); this.F.set(3, 5, deltatime); // Velocity (Y)
        this.F.set(4, 4, 1);  // Acceleration (X)
        this.F.set(5, 5, 1);  // Acceleration (Y)
